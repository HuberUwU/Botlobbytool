document.addEventListener('DOMContentLoaded', () => {
  ...
  const gameSelectionInput = document.getElementById('game-selection'); // Obtener el input del juego
  ...
  startButton.addEventListener('click', () => {
    // Incluir el juego seleccionado en la configuración
    const config = {
      mode: gameMode.value,
      viewCount: parseInt(viewCount.value),
      game: gameSelectionInput.value // Añadir el juego seleccionado
    };
    
    // Animer le bouton pour montrer le clic
    startButton.classList.add('clicked');
    
    // Envoyer la configuración completa (incluyendo el juego) al proceso principal
